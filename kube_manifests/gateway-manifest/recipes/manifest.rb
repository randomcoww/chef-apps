...
table ip filter {
  ...
  chain forward {
    ...
    iifname $if_internal ip daddr #{ip_passthrough} accept;
    iifname $if_internal ip daddr #{subnet_metallb} accept;

    ip daddr #{ip_passthrough} ct status dnat accept;
  }
...
table ip nat {
  ...
  chain prerouting {
    ...
    tcp dport @tcp_dnat dnat #{ip_passthrough};
  }
