...
node.default['qemu']['dns2']['systemd_config'] = {
  '/etc/systemd/network/eth0.network' => {
    ...
    "Address" => {
      "Address" => "#{node['environment_v2']['host']['dns2']['ip_lan']}/#{node['environment_v2']['subnet']['lan'].split('/').last}"
    },
    "Route" => {
  ...
  '/etc/systemd/system/chef-client.service' => {
    ...
    "Service" => {
      "Type" => "oneshot",
      "ExecStart" => "/usr/bin/chef-client -o #{node['qemu']['dns2']['chef_recipes'].join(',')}",
