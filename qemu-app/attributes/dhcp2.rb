node.default['qemu']['dhcp2']['cloud_config_hostname'] = 'dhcp2'
node.default['qemu']['dhcp2']['cloud_config_path'] = "/img/cloud-init/#{node['qemu']['dhcp2']['cloud_config_hostname']}"

node.default['qemu']['dhcp2']['networking'] = {
  '/etc/systemd/network/eth0.network' => {
    ...
    "Address" => {
      "Address" => "#{node['environment_v2']['dhcp2_lan_ip']}/#{node['environment_v2']['lan_subnet'].split('/').last}"
    },
    "Route" => {
  ...
  '/etc/systemd/network/eth1.network' => {
    ...
    "Address" => {
      "Address" => "#{node['environment_v2']['dhcp2_vpn_ip']}/#{node['environment_v2']['store_subnet'].split('/').last}"
    }
  },
...
node.default['qemu']['dhcp2']['chef_recipes'] = [
  "recipe[cql-app::kea_leases]",
  "recipe[kea-app::cql]"
]
node.default['qemu']['dhcp2']['cloud_config'] = {
  "write_files" => [],
  "password" => "password",
  "chpasswd" => {
    "expire" => false
...
node.default['qemu']['dhcp2']['libvirt_config'] = {
  "domain"=>{
    "#attributes"=>{
      "type"=>"kvm"
    },
    "name"=>node['qemu']['dhcp2']['cloud_config_hostname'],
    "memory"=>{
      "#attributes"=>{
        "unit"=>"GiB"
      },
    ...
    "devices"=>{
      ...
      "disk"=>{
        ...
        "source"=>{
          "#attributes"=>{
            "file"=>"/img/kvm/#{node['qemu']['dhcp2']['cloud_config_hostname']}.qcow2"
          }
        },
        "target"=>{
          "#attributes"=>{
      ...
      "filesystem"=>[
        ...
        },
        {
          ...
          "source"=>{
            "#attributes"=>{
              "dir"=>node['qemu']['dhcp2']['cloud_config_path']
            }
          },
          "target"=>{
            "#attributes"=>{
