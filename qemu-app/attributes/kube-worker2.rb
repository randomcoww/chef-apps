node.default['qemu']['kube-worker2']['cloud_config_hostname'] = 'kube-worker2'
node.default['qemu']['kube-worker2']['cloud_config_path'] = "/img/cloud-init/#{node['qemu']['kube-worker2']['cloud_config_hostname']}"

node.default['qemu']['kube-worker2']['chef_recipes'] = [
  "recipe[system_update::debian]",
  "recipe[kubernetes-app::kube_worker]",
]

node.default['qemu']['kube-worker2']['systemd_config'] = {
  '/etc/systemd/network/eth0.network' => {
    ...
    "Address" => {
      "Address" => "#{node['environment_v2']['host']['kube-worker2']['ip_lan']}/#{node['environment_v2']['subnet']['lan'].split('/').last}"
    },
    "Route" => {
  ...
  '/etc/systemd/system/chef-client.service' => {
    ...
    "Service" => {
      "Type" => "oneshot",
      "ExecStart" => "/usr/bin/chef-client -o #{node['qemu']['kube-worker2']['chef_recipes'].join(',')}",
...
node.default['qemu']['kube-worker2']['cloud_config'] = {
  "write_files" => [],
  "password" => "password",
  "chpasswd" => {
    "expire" => false
  ...
  "fqdn" => "#{node['qemu']['kube-worker2']['cloud_config_hostname']}.lan",
  "runcmd" => [
    ...
    [
      "chef-client", "-o",
      node['qemu']['kube-worker2']['chef_recipes'].join(',')
    ],
...
node.default['qemu']['kube-worker2']['libvirt_config'] = {
  "domain"=>{
    "#attributes"=>{
      "type"=>"kvm"
    },
    "name"=>node['qemu']['kube-worker2']['cloud_config_hostname'],
    "memory"=>{
      "#attributes"=>{
        "unit"=>"GiB"
      },
    ...
    "devices"=>{
      ...
      "disk"=>{
        ...
        "source"=>{
          "#attributes"=>{
            "file"=>"/img/kvm/#{node['qemu']['kube-worker2']['cloud_config_hostname']}.qcow2"
          }
        },
        "target"=>{
          "#attributes"=>{
      ...
      "filesystem"=>[
        ...
        },
        {
          ...
          "source"=>{
            "#attributes"=>{
              "dir"=>node['qemu']['kube-worker2']['cloud_config_path']
            }
          },
          "target"=>{
            "#attributes"=>{
