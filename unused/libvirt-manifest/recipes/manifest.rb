...
guests.each do |guest, host|
  ...
  libvirt_config = LibvirtConfig::ConfigGenerator.generate_from_hash({
    "domain"=>{
      "#attributes"=>{
        "type"=>"kvm"
      },
  ...
  manifest = {
    ...
    "spec" => {
      ...
      "containers" => [
        {
          "name" => "libvirt-#{guest}",
          "image" => node['kube']['images']['libvirt_monitor'],
          "volumeMounts" => [
            {
              "name" => "data-kvm",
              "mountPath" => "/data/kvm"
  ...
  node.default['kubernetes']['static_pods'][host]["libvirt-#{guest}"] = manifest
end
